...
def main():
    ...
    print("           ******       Current (2024) Secret Santa assignments:        ****** \n")
    for employee in secret_santa_manager.employees:
        print(f"{employee.employee_name} ({employee.employee_email}) -> {employee.secret_santa.employee_name} ({employee.secret_santa.employee_email})")
    print("\n\n")
    
    df = pd.DataFrame({
        'Employee_Name': [emp.employee_name for emp in secret_santa_manager.employees],
        'Employee_EmailID': [emp.employee_email for emp in secret_santa_manager.employees],
        'Secret_Child_Name': [emp.secret_santa.employee_name for emp in secret_santa_manager.employees],
        'Secret_Child_EmailID': [emp.secret_santa.employee_email for emp in secret_santa_manager.employees]
    })
